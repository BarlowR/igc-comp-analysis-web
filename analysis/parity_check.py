...
using the prototype's geometry + τ, fed the app's own constants (V_g, g, M,
h_fin, pace — all shipped in timeToGo), and diff them — so any gap is a genuine
logic divergence between the two implementations, not a difference in how the
day constants were derived.

    python3 analysis/parity_check.py                 # all built days
    python3 analysis/parity_check.py dist/archive/chelan2026/day3.json

Must match competition.ts: smoothAlt window 7 s; τ pre-multiplied by pace.
"""
import glob
import json
import sys
...
def tau_of(rem, h, ttg):
    """τ per fix (minutes) — the exact shipped formula: pace-fitted two-phase
    time, tauSeries (timetogo.ts) × pace (competition.ts)."""
    V_g, g, M, h_fin, pace = ttg["Vg"], ttg["g"], ttg["M"], ttg["hFin"], ttg["pace"]
    out = []
    for i in range(len(rem)):
        d = rem[i]
        owed = max(0.0, d / g - (h[i] - h_fin))
        out.append(pace * (d / V_g + owed / M) / 60)
    return out
